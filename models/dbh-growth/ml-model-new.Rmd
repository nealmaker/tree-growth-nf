...
subtitle: "using a machine learning approach and a curated list of covariates"
author: "Neal Maker"
...
nf_fia <- nf_fia %>%
  filter(status_change == "lived", !is.na(spp_rich_mid), 
         !is.na(dbh_var_mid)) %>%
  select(dbh_rate, spp, dbh_mid, cr_mid, bal_mid, spp_rich_mid, 
         dbh_var_mid, forest_type_s, landscape, site_class, lat, lon,
         ba_ash:ba_yellow.birch, interval, plot) %>%
  rename(dbh = dbh_mid, cr = cr_mid, bal = bal_mid,
         spp_rich = spp_rich_mid, dbh_var = dbh_var_mid, 
         forest_type = forest_type_s)
})})
```
...
The data include `r ncol(nf_fia) - 2` potential predictors (features), which include individual tree attributes as well as plot-level attributes and site attributes. Three of the predictors are non-ordinal categorical variables, one is an ordinal categorical variable, and the rest are numeric. Many of the numeric predictors are species-specific basal areas. In aggregate, these fully describe basal area for each observation and total basal area was omitted from the covariates to prevent perfect collinearities.
...
dbh_recipe1 <- recipe(dbh_rate ~ ., data = train) %>% 
  step_YeoJohnson("dbh", "cr", "bal", "spp_rich", "dbh_var",
                  "site_class", "lat", "lon") %>% 
  step_center("dbh", "cr", "bal", "spp_rich", "dbh_var",
                  "site_class", "lat", "lon") %>% 
  step_scale("dbh", "cr", "bal", "spp_rich", "dbh_var",
                  "site_class", "lat", "lon") %>% 
...
Finally, highly correlated predictors were filtered, to cut down on features to speed training and prevent problems with linear regression-based methods. The only features that are potentially highly correlated are basal areas specific to uncommon species. For example, the basal area of cottonwood is approximately 95% correlated in the training data, because the only cottonwoods observed came from a few cottonwood filled plots.
...
invisible({capture.output({
  ...
  fit.gbm1 <- train(dbh_recipe, train, method = 'gbm', metric=metric,
                    ...
                    tuneGrid = expand.grid(shrinkage = .5,
                                           n.trees = seq(2000, 80000, by = 2000),
                                           interaction.depth = 3, 
                                           n.minobsinnode = 10)) 
...
```{r gbmLR}
# fit.gbm2 TOOK SEVERAL DAYS. LOWERING LEARNING RATE WILL BE EVEN SLOWER.
  ...
  fit.gbm3 <- 
    train(dbh_recipe, train, method = 'gbm', metric=metric,
          ...
          tuneGrid = expand.grid(shrinkage = 0.01,
                                 n.trees = c(20000, 40000, 60000, 80000),
                                 interaction.depth = 
...
# redo b/c didn't reach minimum RMSE
invisible({capture.output({
  cl <- makePSOCKcluster(10)
  registerDoParallel(cl)
  ...
  fit.gbm4 <- 
    train(dbh_recipe, train, method = 'gbm', metric=metric,
          trControl=control2, verbose=FALSE,
          tuneGrid = expand.grid(shrinkage = 0.01,
                                 n.trees = c(80000, 100000, 120000, 140000),
                                 interaction.depth = 
                                   fit.gbm2$bestTune$interaction.depth,
                                 n.minobsinnode = 
                                   fit.gbm2$bestTune$n.minobsinnode))
...
invisible({capture.output({
  ...
  fit.gbm5 <- 
    train(dbh_recipe, train, method = 'gbm', metric=metric,
          trControl=control2, verbose=FALSE,
          tuneGrid = expand.grid(shrinkage = 0.01,
                                 n.trees = c(160000, 200000, 240000),
                                 interaction.depth = 4,
                                   #fit.gbm2$bestTune$interaction.depth,
                                 n.minobsinnode = 20))
                                   #fit.gbm2$bestTune$n.minobsinnode))
...
invisible({capture.output({
  ...
  fit.gbm.final <- 
    train(dbh_recipe, train, method = 'gbm', metric=metric,
          ...
          tuneGrid = expand.grid(shrinkage = fit.gbm6$bestTune$shrinkage,
                                 n.trees = fit.gbm6$bestTune$n.trees,
                                 interaction.depth = 
                                   fit.gbm6$bestTune$interaction.depth,
                                 n.minobsinnode = 
                                   fit.gbm6$bestTune$n.minobsinnode))
...
This final version of the gradient boosting machine achieves a cross-validation RMSE of `r round(min(fit.gbm6$results[,5]), 4)` inches. It uses `r round(lobstr::obj_size(fit.gbm.final)/1000000)` megabytes in memory, and takes an average of `r round(mean(gbmTimes), 2)` seconds to make one thousand predictions.

``` {r save}
saveRDS(fit.gbm.final, file = "dbh-growth-model-gbm.rds")
```

# Lasso Regression as a Slim Alternative

The results above show that the gradient boosting machine did not scale well when applied to the full training set. While the accuracy did improve a fair bit compared to the model trained on a subset of the data, the model's size and prediction time increased many-fold, creating a model that will be difficult to use. Below, we train a regularized parametric model--quantile regression with lasso penalty--on all of the training data, which we expect will be substantially smaller and faster than the gradient boosting machine, but not as accurate.
...
invisible({capture.output({
  ...
  set.seed(seed)
  fit.lasso1 <- 
    train(dbh_recipe, train, method = 'rqlasso', metric=metric,
          trControl=control, verbose=FALSE,
          tuneGrid = expand.grid(lambda = seq(0, .005, .001)))
...
invisible({capture.output({
  ...
  fit.lasso.final <- 
    train(dbh_recipe, train, method = 'rqlasso', metric=metric,
          trControl = trainControl(method = "none"), verbose = FALSE,
          tuneGrid = expand.grid(lambda = fit.lasso1$bestTune$lambda))
...
lassoTimes <- 
  sapply(1:30, function(k) {
    system.time(predict(fit.lasso.final,
                        newdata = train[sample(1:nrow(train), 1000,
                                               replace = F), ]))[[3]]
  })
```

This final version of quantile regression with lasso achieves a cross-validation RMSE of `r round(min(fit.lasso1$results[,2]), 4)` inches. It uses `r round(lobstr::obj_size(fit.lasso.final)/1000000)` megabytes in memory, and takes an average of `r round(mean(lassoTimes), 2)` seconds to make one thousand predictions.

``` {r saveLasso}
saveRDS(fit.lasso.final, file = "dbh-growth-model-lasso.rds")
```

As expected, the lasso is much smaller and faster than the gradient boosting machine (`r round(lobstr::obj_size(fit.gbm.final)/lobstr::obj_size(fit.lasso.final),1)` times smaller and `r round(mean(gbmTimes)/mean(lassoTimes))` times faster), but is not quite as accurate (with an RMSE of `r round(min(fit.lasso1$results[,2]), 4)` inches, compared to the boosting machine's RMSE of `r round(min(fit.gbm6$results[,5]), 4)` inches). 

The disadvantage of the lasso is most likely its inability to account for interactions between predictors. While the gradient boosting machine's advantage in overall accuracy is relatively small, Maker and Foppert (2021) demonstrated that complex interactions do drive diameter growth trends for trees in less common situations and that nonparametric models like the gradient boosting machine can make much better predictions in those cases. In other words, the gradient boosting machine is probably making a big difference for a minority of trees. 

# Compromising With a Random Forest

Results from the spot checks indicate that a random forest could offer a reasonable compromise between the gradient boosting machine and the lasso regression. Random forests are nonparametric and capable of accounting for interactions, which should help the accuracy, and I suspect they scale better than the gradient boosting machine, hopefully keeping the size and prediction speed down.
...
invisible({capture.output({
  ...
  set.seed(seed)
  fit.rf1 <- 
    caret::train(dbh_recipe, train, method = 'ranger', metric=metric,
          trControl=control2, verbose=FALSE)
...
invisible({capture.output({
  ...
  set.seed(seed)
  fit.rf.final <- 
    train(dbh_recipe, train, method = 'ranger', metric=metric,
          trControl = trainControl(method = "none"), verbose = FALSE,
          tuneGrid = expand.grid(mtry = fit.rf1$bestTune$mtry,
                                 min.node.size = fit.rf1$bestTune$min.node.size,
                                 splitrule = fit.rf1$bestTune$splitrule))
...
rfTimes <- 
  sapply(1:30, function(k) {
    system.time(predict(fit.rf.final,
                        newdata = train[sample(1:nrow(train), 1000,
                                               replace = F), ]))[[3]]
  })
```

This final version of the linear support vector machine achieves a cross-validation RMSE of `r round(min(fit.rf1$results[,2]), 4)` inches. It uses `r round(lobstr::obj_size(fit.rf.final)/1000000)` megabytes in memory, and takes an average of `r round(mean(rfTimes), 2)` seconds to make one thousand predictions.

``` {r saveRf}
saveRDS(fit.rf.final, file = "dbh-growth-model-rf.rds")
```
