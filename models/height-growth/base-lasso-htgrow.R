...
set.seed(seed)
ht_mod_base <- train(x, y, method = 'rqlasso', 
                      metric=metric, 
...
set.seed(seed)
ht_mod_2 <- train(x, y, method = 'rqlasso',
                   metric=metric,
                   preProc=c("center", "scale"),
                   trControl=control,
...
set.seed(seed)
htgrow_mod_lasso <- train(features, ht_rates, method = 'rqlasso',
                       metric=metric,
                       preProc=c("center", "scale"),
                       trControl=control2,
                       tuneGrid = ht_mod_2$bestTune)

stopCluster(cl)

htgrow_mod_dummies <- dummies
save(htgrow_mod_lasso, htgrow_mod_dummies,
     file = "../base-models-rf/htgrow-mod-lasso.rda")
