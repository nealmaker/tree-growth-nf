...
1. Spot check numerous algorithms for modeling tree height, which represent a diversity of methodologies. Focus on:
  - model accuracy
...
nf_fia <- nf_fia %>%
  ...
  select(ht_s, spp, dbh_s, cr_s, crown_class_s, tree_class_s,
         ...
         plot) %>% 
  rename(ht = ht_s, dbh = dbh_s, cr = cr_s, crown_class = crown_class_s,
...
dummies <- dummyVars(ht ~ ., data = nf_fia)
...
```
There was little difference in accuracy between most of the algorithms in the spot check. Interestingly, the neural networks (nnet and mlp) were an exception, performing noticeably worse than the other algorithms. The cubist, random forest, and gradient boosting machines were the most accurate overall, but a host of other algorithms were very close behind.

Spot checked algorithms were more stratified in relation to prediction time and model size. L1- and L2-regularized models were generally the fastest and smallest, while gradient boosting machines, regularized general linear models, and linear support vector machines occupied a middle ground. Radial support vector machines, k-nearest neighbor, and cubist were notably slow. Random forests and bagged regression trees were notably large.

##  Promising Algorithms

To further winnow possible algorithms, four of the most promising were explored in more depth: the gradient boosting machine gbm, the multivariate adaptive regression spline earth, the linear support vector machine svmLinear, and the lasso. These were trained on all of the training data, and a grid search was used to optimize their tuning parameters. Each was tried with and without feature selection (fs), which consisted of removing predictors with near-zero variance and removing highly correlated predictors. The lasso was trained with one-hot encoding and feature selection, like the other models, and also without one-hot encoding using species-specific submodels to fully capture species effects.
...
# BOTH FAILED WITH ERR: all RMSE metric values are missing ###################################################

# set.seed(seed)
# fit.earth <- train(train, trainy, method = 'earth', metric=metric, 
#                  preProc=c("center", "scale"), trControl=control, verbose=FALSE,
#                  tuneLength = 5) # number of values for each tuning param
# 
# fit.earth.fs <- train(train_fs, trainy, method = 'earth', metric=metric, 
#                  preProc=c("center", "scale"), trControl=control, verbose=FALSE,
#                  tuneLength = 5)
...
#### RAN FOR MORE THAN A DAY (whole invisible chunk) AND I TERMIINATED ########################################
...
# 
# set.seed(seed)
# fit.svmLinear <- train(train, trainy, method = 'svmLinear', metric=metric, 
#                  preProc=c("center", "scale"), trControl=control, verbose=FALSE,
#                  tuneLength = 5) # number of values for each tuning param
# 
# fit.svmLinear.fs <- train(train_fs, trainy, method = 'svmLinear', metric=metric, 
#                  preProc=c("center", "scale"), trControl=control, verbose=FALSE,
#                  tuneLength = 5)
# 
# stopCluster(cl)
...
fits <- list(
  ...
  gbm_fs = fit.gbm.fs,
  earth = fit.earth,
  earth_fs = fit.earrh.fs,
  svmLinear = fit.nnet,
  svmLinear_fs = fit.nnet.fs,
